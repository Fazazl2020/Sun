...
=====================================================
InstanceNorm + MagLoss(3.0), F=201

Loss: SI-SDR + 10.0 * L1(R,I) + 3.0 * MSE(Magnitude)

CHANGES FROM S1:
- STFT: win=400 (25ms), hop=100 (6.25ms) -> F=201 (was F=161)
- 25% more frequency bins for finer spectral detail
- Matches CMGAN's STFT configuration

USAGE:
1. Copy this file to configs.py: cp configs_s2.py configs.py
...
EXPERIMENT_DESC = 'InstanceNorm + MagLoss(3.0), F=201 (higher resolution)'
...
MODEL_CONFIG = {
    'in_norm': False,
    'sample_rate': 16000,
    # S2: Higher STFT resolution (F=201) - matches CMGAN
    'win_len': 0.025,       # 400 samples -> F=201
    'hop_len': 0.00625,     # 100 samples
    # S2: Standard magnitude loss (no compression)
