"""
networks_ab6_full.py - Transformer Full Fix
============================================
Ab6-full = Ab5 (FAC Fix) + Transformer Full Fixes

Transformer Fixes (comprehensive, ~40% overhead):
1. Cosine temperature: Learnable temperature for sharper attention
2. Transformer PE: Time and frequency positional encoding
3. FFN: Feed-forward network after attention (standard transformer)
4. Sequential T-F: Time attention sees frequency-processed features
...
Expected improvement: +0.15 to +0.28 PESQ
Parameter overhead: ~262K params (~56%)
MAC overhead: ~40%
...
class Hybrid_SelfAttention_MRHA3(nn.Module):
    ...
    FULL FIXES:
    1. Learnable temperature for cosine branch
    2. Feed-forward network after attention (standard transformer component)
    """
    def __init__(self, in_channels, downsample_stride=2, ffn_expansion=4, dropout=0.1):
        super().__init__()
        ...
        # ===========================================
        # FIX 3: Feed-Forward Network (FFN)
        # ===========================================
        # Standard transformer FFN: Linear -> GELU -> Dropout -> Linear -> Dropout
        # Expansion factor 4 is standard (128 -> 512 -> 128)
    ...
    def forward(self, x):
        ...
        # FIX 1: Divide by learnable temperature
        ...
        # FIX 3: Apply FFN with residual connection
        # Standard transformer: z = z + FFN(LayerNorm(z))
        z = z + self.ffn(self.ffn_norm(z))

        return z


class AIA_Transformer_Sequential(nn.Module):
    """
    ...
    def __init__(self, input_size, output_size, dropout=0.1):
        ...
        # Separate transformers for row (freq) and col (time)
    ...
    def forward(self, x):
        ...
        # ===========================================
        # STEP 1: Frequency attention first (row-wise)
        # Each time frame attends across frequency bins
        ...
        # Update x with frequency-attended features
        x = x + self.k1 * row_out

        # STEP 2: Time attention second (column-wise)
        # Now time attention sees frequency-contextualized features!
        # This allows temporal modeling to leverage frequency patterns
        ...
        # Final output with time attention
        out = x + self.k2 * col_out
...
class Net(nn.Module):
    """
    Speech Enhancement Network - Ab6-full (Transformer Full Fix)

    Ab6-full = Ab5 + Transformer Full Fixes

    Transformer Fixes:
    1. Cosine temperature: Learnable temperature for sharper attention
    2. Transformer PE: Time and frequency positional encoding
    3. FFN: Feed-forward network after attention
    4. Sequential T-F: Frequency first, then time (better info flow)
    ...
    def __init__(self, F=201):
        ...
        # AIA Transformer with FULL fixes (Sequential T-F)
        self.m1 = AIA_Transformer_Sequential(256, 256)
    ...
    def forward(self, x, global_step=None):
        ...
        # AIA with FULL transformer fixes
        aia_out = self.m1(e5)
