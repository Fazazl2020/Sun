...
class AdaptiveFrequencyBandPositionalEncoding(nn.Module):
    def __init__(self, strategy='learnable-sin', F=801, sample_rate=16000):
        super().__init__()
    ...
    def forward(self, X):
        ...
        if self.pe_low is not None and current_low > 0:
            pe_low = F.interpolate(self.pe_low.unsqueeze(0).unsqueeze(0),
                                   size=current_low, mode='linear', align_corners=False)
            pe_low = pe_low.unsqueeze(2)  # Add time dimension at dim 2: [1, 1, 1, current_low]
        ...
        if self.pe_mid is not None and current_mid > 0:
            pe_mid = F.interpolate(self.pe_mid.unsqueeze(0).unsqueeze(0),
                                   size=current_mid, mode='linear', align_corners=False)
            pe_mid = pe_mid.unsqueeze(2)  # Add time dimension at dim 2
            # FIXED: Apply to dim 3 (frequency), not dim 2 (time)
        ...
        if self.pe_high is not None and current_high > 0:
            pe_high = F.interpolate(self.pe_high.unsqueeze(0).unsqueeze(0),
                                    size=current_high, mode='linear', align_corners=False)
            pe_high = pe_high.unsqueeze(2)  # Add time dimension at dim 2
            # FIXED: Apply to dim 3 (frequency), not dim 2 (time)
...
class DepthwiseFrequencyAttention(nn.Module):
    ...
    Input shape: [B, C, T, F] where T=time, F=frequency
    """
    def __init__(self, in_channels, kernel_size=5):
        super().__init__()
        # FIXED: kernel (1, kernel_size) operates on dim 3 (frequency), not dim 2 (time)
        # Was: (kernel_size, 1) which operated on time
        self.dw_conv = nn.Conv2d(in_channels, in_channels,
                               kernel_size=(1, kernel_size),  # FIXED: (time=1, freq=5)
                               padding=(0, kernel_size//2),   # FIXED: padding on freq dim
                               groups=in_channels)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        # x: [B, C, T, F] - attention computed along F (frequency)
        att_mask = self.sigmoid(self.dw_conv(x))
        return x * att_mask

class GatedPositionalEncoding(nn.Module):
    def __init__(self, in_channels, F=301, sample_rate=16000):
        super().__init__()
...
class FACLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, F=None, sample_rate=16000):
        super().__init__()
        reduction = 8 if in_channels >= 16 else 1
...
class Net(nn.Module):
    def __init__(self, F=161):
        ...
        self.conv3 = FACLayer(32, 64, (2,3), (1,2), (1,0), F)
        self.conv4 = FACLayer(64, 128, (2,3), (1,2), (1,0), F)
        self.conv5 = FACLayer(128, 256, (2,3), (1,2), (1,0), F)
...
class Hybrid_SelfAttention_MRHA3(nn.Module):
    def __init__(self, in_channels, downsample_stride=2):
        ...
        self.downsample = nn.Conv1d(
            ...
            kernel_size=3, stride=downsample_stride, padding=1
        )
    ...
    def forward(self, x):
        ...
        # --- 3-Way Gating ---
        fused = torch.cat([out_cross, out_dot, out_cos], dim=-1).permute(0,2,1)  # (B,3C,T)
        gating = F.softmax(self.gate_conv(fused), dim=1)  # (B,3,T)
        gating = gating.permute(0,2,1).unsqueeze(2)  # Fixed: (B,T,1,3)

        # --- Weighted Sum ---
        outputs = torch.stack([out_cross, out_dot, out_cos], dim=3)  # (B,T,C,3)
        z = torch.sum(gating * outputs, dim=3)  # Now shapes: (B,T,1,3) * (B,T,C,3) ? sum ? (B,T,C)
        return z
